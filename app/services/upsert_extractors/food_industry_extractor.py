...
def transform_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    possible_column_names = {
        'license_number': [
            'License Number', 'License No', 'License No.', 'License', 'LTO Number',
            'license number', 'license no', 'license', 'lto number', 'licenseno',
            'LICENSE NUMBER', 'LICENSE NO', 'LICENSE', 'LTO NUMBER', 'Lic No', 'lic no'
        ],
        'name_of_establishment': [
            'Name of Establishment', 'Establishment Name', 'Establishment', 'Company Name',
            'name of establishment', 'establishment name', 'establishment', 'company name',
            'NAME OF ESTABLISHMENT', 'ESTABLISHMENT NAME', 'ESTABLISHMENT', 'COMPANY NAME',
            'Name', 'name', 'NAME', 'Business Name', 'business name'
        ],
        'owner': [
            'Owner', 'Proprietor', 'Owner Name', 'Owned By',
            'owner', 'proprietor', 'owner name', 'owned by',
            'OWNER', 'PROPRIETOR', 'OWNER NAME', 'OWNED BY'
        ],
        'address': [
            'Address', 'Location', 'Business Address', 'Complete Address',
            'address', 'location', 'business address', 'complete address',
            'ADDRESS', 'LOCATION', 'BUSINESS ADDRESS', 'COMPLETE ADDRESS'
        ],
        'region': [
            'Region', 'Regional Location', 'Area',
            'region', 'regional location', 'area',
            'REGION', 'REGIONAL LOCATION', 'AREA'
        ],
        'activity': [
            'Activity', 'Business Activity', 'Type of Activity', 'Business Type',
            'activity', 'business activity', 'type of activity', 'business type',
            'ACTIVITY', 'BUSINESS ACTIVITY', 'TYPE OF ACTIVITY', 'BUSINESS TYPE'
        ],
        'issuance_date': [
            ...
            'ISSUANCE DATE', 'ISSUE DATE', 'DATE OF ISSUE', 'DATE ISSUED', 
            'ISSUED DATE', 'Date of Issuance', 'date of issuance'
        ],
        'expiry_date': [
            ...
            'EXPIRY DATE', 'EXPIRATION DATE', 'EXP DATE', 'VALID UNTIL', 
            'EXPIRES', 'Validity Date', 'validity date'
        ]
    }
    ...
    required_fields = ['license_number', 'name_of_establishment', 'owner', 'address', 
                      'region', 'activity', 'issuance_date', 'expiry_date']
    ...
    if 'license_number' in df_clean.columns:
        ...
        logger.info(f"Removed {initial_count - len(df_clean)} rows with missing license_number")
    ...
    if duplicates.any():
        ...
        logger.warning(f"Found {num_duplicates} duplicate license numbers, keeping first occurrence")
    ...
    if 'issuance_date' in df_clean.columns and 'expiry_date' in df_clean.columns:
        ...
        if invalid_dates.any():
            ...
            logger.warning(f"Found {num_invalid} rows with expiry_date <= issuance_date")
            # Log a few examples
            invalid_samples = df_clean[invalid_dates][['license_number', 'issuance_date', 'expiry_date']].head()
            logger.warning(f"Examples:\n{invalid_samples}")
    
    # Check for required fields with None values
    required_not_null = ['name_of_establishment', 'owner', 'address', 'region', 
                         'activity', 'issuance_date', 'expiry_date']
    
    for field in required_not_null:
        if field in df_clean.columns:
            null_count = df_clean[field].isna().sum()
            if null_count > 0:
                logger.warning(f"Field '{field}' has {null_count} null values ({null_count/len(df_clean)*100:.1f}%)")
    
    # Replace pandas NaT and NaN with None for database compatibility
    df_clean = df_clean.where(pd.notnull(df_clean), None)
    
    logger.info(f"✅ Cleaned data: {len(df_clean)} rows (from {initial_count} initial rows)")
        ...
        async with get_session_with_rollback() as session:
            for i in range(0, len(data), batch_size):
                ...
                try:
                    ...
                    for idx, record in enumerate(batch):
                        try:
                            ...
                            if not record.get('license_number'):
                                logger.warning(f"Skipping record {i+idx}: missing license_number")
                                failed_records.append(record)
                                total_failed += 1
                                continue
                            
                            # Ensure all required fields are present
                            required_fields = ['license_number', 'name_of_establishment', 
                                             'owner', 'address', 'region', 'activity',
                    ...
                    if not valid_batch:
                        logger.warning(f"Batch {batch_num} has no valid records, skipping")
                        continue
                    ...
                    logger.info(f"✅ Batch {batch_num}/{total_batches} processed ({len(valid_batch)} rows)")
    ...
    try:
        async with async_session() as session:
            result = await session.execute(
                select(FoodIndustry).limit(limit)
            )
            ...
            if rows:
                ...
                for row in rows:
                    logger.info(f"  - {row.license_number}: {row.name_of_establishment}")
            else:
...
async def process_single_file(file_path: str, use_upsert: bool = True):
    ...
    try:
        ...
        if df_clean.empty:
            logger.warning("⚠️  No valid data after cleaning, skipping database insertion")
            return
