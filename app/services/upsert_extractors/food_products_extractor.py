...
def transform_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    possible_column_names = {
        'registration_number': [
            'Registration Number', 'Registration No', 'Registration No.', 'Reg Number',
            'registration number', 'registration no', 'reg number', 'registrationno',
            'REGISTRATION NUMBER', 'REGISTRATION NO', 'REG NUMBER', 'Reg No', 'reg no',
            'Certificate Number', 'certificate number', 'Cert Number', 'cert number'
        ],
        'company_name': [
            'Company Name', 'Company', 'Applicant', 'Manufacturer', 'Applicant Company',
            'company name', 'company', 'applicant', 'manufacturer', 'applicant company',
            'COMPANY NAME', 'COMPANY', 'APPLICANT', 'MANUFACTURER', 'APPLICANT COMPANY',
            'Name of Company', 'name of company', 'Business Name', 'business name'
        ],
        'product_name': [
            'Product Name', 'Product', 'Name of Product', 'Generic Name',
            'product name', 'product', 'name of product', 'generic name',
            'PRODUCT NAME', 'PRODUCT', 'NAME OF PRODUCT', 'GENERIC NAME',
            'Item Name', 'item name', 'Name', 'name'
        ],
        'brand_name': [
            'Brand Name', 'Brand', 'Trade Name', 'Trademark',
            'brand name', 'brand', 'trade name', 'trademark',
            'BRAND NAME', 'BRAND', 'TRADE NAME', 'TRADEMARK',
            'Commercial Name', 'commercial name'
        ],
        'type_of_product': [
            'Type of Product', 'Product Type', 'Type', 'Category', 'Classification',
            'type of product', 'product type', 'type', 'category', 'classification',
            'TYPE OF PRODUCT', 'PRODUCT TYPE', 'TYPE', 'CATEGORY', 'CLASSIFICATION',
    ...
    required_fields = ['registration_number', 'company_name', 'product_name', 
                      'brand_name', 'type_of_product', 'issuance_date', 'expiry_date']
    ...
    if 'registration_number' in df_clean.columns:
        ...
        logger.info(f"Removed {initial_count - len(df_clean)} rows with missing registration_number")
    ...
    if duplicates.any():
        ...
        logger.warning(f"Found {num_duplicates} duplicate registration numbers, keeping first occurrence")
    ...
    if 'issuance_date' in df_clean.columns and 'expiry_date' in df_clean.columns:
        ...
        if invalid_dates.any():
            ...
            invalid_samples = df_clean[invalid_dates][['registration_number', 'issuance_date', 'expiry_date']].head()
            logger.warning(f"Examples:\n{invalid_samples}")
    
    # Check for required fields with None values
    required_not_null = ['company_name', 'product_name', 'brand_name', 
                         'type_of_product', 'issuance_date', 'expiry_date']
                    ...
                    for idx, record in enumerate(batch):
                        try:
                            ...
                            if not record.get('registration_number'):
                                logger.warning(f"Skipping record {i+idx}: missing registration_number")
                            ...
                            required_fields = ['registration_number', 'company_name', 
                                             'product_name', 'brand_name', 'type_of_product',
    ...
    try:
        async with async_session() as session:
            result = await session.execute(
                select(FoodProducts).limit(limit)
            )
